...
my, my_sigma = twodspec.ap_trace(
    img, nsteps=20, spatial_mask=spatial_mask, cosmic=True, n_spec=1,
    recenter=False, prevtrace=(0, ), fittype=fittype, order=order,
    bigbox=8, Saxis=Saxis, silence=True, display=False
    )
...
if Saxis == 1:
    my_json = json.dumps([{"mu": my[0][i], "sig": my_sigma[0][i]} for i in range(x_len)])
else:
    my_json = json.dumps([{"mu": my[0][i], "sig": my_sigma[0][i]} for i in range(y_len)])
